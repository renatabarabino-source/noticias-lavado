...
# Timeout estricto para evitar que el proceso se trabe
...
# Lista completa de portales (Nacionales + Internacionales)
PORTALES_PRENSA = (
...
def fetch_category(query):
    # Buscamos con foco en Argentina pero permitiendo resultados globales
    ...
    for entry in entries:
        ...
        # Filtro: Evitar links vacíos a homes de portales
...
# --- EJECUCIÓN DE LAS 2 SOLAPAS ---

# 1. ACTUALIZACIONES (Normativas): Sitios oficiales + portales si hablan de leyes/resoluciones
q_actualizaciones = f'({BASE_AML}) AND (({SITES_GOV}) OR (({PORTALES_PRENSA}) AND ("normativa" OR "resolución" OR "arca" OR "uif" OR "gafi" OR "cnv" OR "ley")))'
news_actualizaciones = fetch_category(q_actualizaciones)

# 2. NOTICIAS (General): Prensa nacional e internacional combinada (incluye Dólar Blue si es por lavado)
q_noticias = f'({BASE_AML} OR "dólar blue") AND ("lavado" OR "blanqueo") AND ({PORTALES_PRENSA})'
news_noticias = fetch_category(q_noticias)
...
<head>
    ...
    <style>
        ...
        #actualizaciones {{ background: linear-gradient(rgba(255,255,255,0.95), rgba(255,255,255,0.95)), url('https://images.unsplash.com/photo-1554224155-1696413575b3?w=800'); background-size: cover; background-attachment: fixed; }}
        #noticias {{ background: linear-gradient(rgba(240,248,255,0.95), rgba(240,248,255,0.95)), url('https://images.unsplash.com/photo-1571171637578-41bc2dd41cd2?w=800'); background-size: cover; }}
...
<body>
    ...
    <div class="tabs">
        <button class="tab-btn active" onclick="showTab('actualizaciones')">ACTUALIZACIONES</button>
        <button class="tab-btn" onclick="showTab('noticias')">NOTICIAS</button>
    </div>
    
    <div id="actualizaciones" class="page active"><div class="grid">{''.join([f'<div class="card"><span class="badge">{n["fuente"]}</span><h3><a href="{n["link"]}" target="_blank">{n["titular"]}</a></h3><p class="desc">{n["resumen"]}</p></div>' for n in news_actualizaciones])}</div></div>
    
    <div id="noticias" class="page"><div class="grid">{''.join([f'<div class="card"><span class="badge">{n["fuente"]}</span><h3><a href="{n["link"]}" target="_blank">{n["titular"]}</a></h3><p class="desc">{n["resumen"]}</p></div>' for n in news_noticias])}</div></div>
